...
/* ════════════════════════════════════════════════════════════
   SHARED ACCOMMODATION PAGE — Browse co-living spaces
...
const GENDER_OPTIONS = ['Any', 'Mixed', 'Female Only', 'Male Only'];
const PER_PAGE = 12;

/* ── Filter Drawer ─────────────────────────────────────────── */
function SharedFilterDrawer({ isOpen, onClose, filters, onChange }) {
    ...
    <div className="fixed inset-0 z-[100] lg:hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 max-h-[85vh] overflow-y-auto bg-white dark:bg-brand-charcoal-dark rounded-t-3xl animate-slide-up">
        ...
        <div className="p-5 space-y-5">
          ...
          {/* Budget range */}
          <div>
            <label className="mb-2 label-sm">Budget per Month</label>
          ...
          <div className="flex items-center gap-3">
            <input type="checkbox" id="sh-available" checked={local.availableOnly}
              onChange={e => set('availableOnly', e.target.checked)}
              className="w-4 h-4 accent-brand-gold" />
            <label htmlFor="sh-available" className="flex items-center gap-2 text-sm font-semibold cursor-pointer text-brand-charcoal-dark dark:text-white">
          ...
          {/* Verified */}
          <div className="flex items-center gap-3">
            <input type="checkbox" id="sh-verified" checked={local.verified}
              onChange={e => set('verified', e.target.checked)}
              className="w-4 h-4 accent-brand-gold" />
            <label htmlFor="sh-verified" className="flex items-center gap-2 text-sm font-semibold cursor-pointer text-brand-charcoal-dark dark:text-white">
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-gray-100 dark:border-white/10">
          <button type="button"
            onClick={() => setLocal({ state:'All States', minPrice:'', maxPrice:'', gender:'Any', availableOnly:false, verified:false })}
...
/* ── Main Page ─────────────────────────────────────────────── */
export default function SharedAccommodation() {
  ...
  const filtered = useMemo(() => {
    let list = properties.filter(p => p.category === 'shared');
    ...
    if (filters.minPrice) {
      ...
      list = list.filter(p => {
        const cheapest = p.rooms?.length
          ? Math.min(...p.rooms.filter(r => r.available).map(r => r.price))
    ...
    if (filters.availableOnly) {
      list = list.filter(p => p.rooms?.some(r => r.available));
    }

    if (filters.verified)
      list = list.filter(p => p.verified);

    // Gender filter (check title/description for keywords)
    if (filters.gender === 'Female Only')
      list = list.filter(p => p.title.toLowerCase().includes('female') || p.description?.toLowerCase().includes('female'));
    if (filters.gender === 'Male Only')
      list = list.filter(p => p.title.toLowerCase().includes('male') && !p.title.toLowerCase().includes('female'));

    if (sortBy === 'price_asc')  list.sort((a, b) => (a.pricePerMonth || a.price) - (b.pricePerMonth || b.price));
    if (sortBy === 'price_desc') list.sort((a, b) => (b.pricePerMonth || b.price) - (a.pricePerMonth || a.price));
  ...
  const activeCount = [
    ...
    filters.gender !== 'Any', filters.availableOnly, filters.verified,
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900/30">
        ...
        <div className="flex items-center max-w-6xl gap-2 mx-auto">
          <div className="relative flex-1">
            ...
            <input type="search" value={keyword}
              ...
              placeholder="Search shared accommodation…"
      ...
      <div className="px-4 py-6 mx-auto max-w-6xl">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-400">
              {filtered.length} co-living space{filtered.length !== 1 ? 's' : ''} available
            </p>
          </div>
        ...
        {/* ── Info banner ────────────────────────────────────── */}
        <div className="flex items-center gap-3 p-3.5 mb-5 bg-purple-50 dark:bg-purple-500/10 rounded-2xl border border-purple-100 dark:border-purple-500/20">
          <Users size={16} className="text-purple-500 shrink-0" />
          <p className="text-xs text-gray-600 dark:text-gray-300">
            <strong className="text-brand-charcoal-dark dark:text-white">Find your community.</strong> All hosts are verified. View housemate profiles and community guidelines before applying.
          </p>
        </div>
        ...
        {paginated.length > 0 ? (
                ...
                {p.rooms && (
                  <div className="absolute top-3 right-12 z-10">
                    <span className={`text-[10px] font-bold px-2 py-1 rounded-full shadow-sm ${
                      p.rooms.filter(r => r.available).length > 0
                        ? 'bg-emerald-500 text-white'
                        : 'bg-gray-400 text-white'
                    }`}>
                      {p.rooms.filter(r => r.available).length} room{p.rooms.filter(r => r.available).length !== 1 ? 's' : ''} free
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="py-16 text-center">
            ...
            <h2 className="mb-1 text-lg font-bold font-display text-brand-charcoal-dark dark:text-white">No shared spaces found</h2>
            <p className="mb-4 text-sm text-gray-400">Try adjusting your search or filters</p>
            <button type="button"
              onClick={() => { setKeyword(''); setFilters({ state:'All States', minPrice:'', maxPrice:'', gender:'Any', availableOnly:false, verified:false }); setPage(1); }}
