        ...
        <div className="flex items-center max-w-6xl gap-2 mx-auto">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 rounded-xl bg-brand-gray-soft dark:bg-white/10 text-sm font-body text-brand-charcoal-dark dark:text-white placeholder:text-gray-400 border border-transparent focus:border-brand-gold outline-none transition-all"
            />
          </div>

          {/* Filters button */}
          <button type="button" onClick={() => setShowFilters(true)}
            className={`flex items-center gap-1.5 px-3.5 py-3 rounded-xl border-2 text-sm font-bold transition-all shrink-0 active:scale-[0.97]
              ${activeFilterCount > 0
          ...
          <div className="flex bg-brand-gray-soft dark:bg-white/10 rounded-xl p-0.5 shrink-0">
            {[
              ...
              <button key={id} type="button" onClick={() => setViewMode(id)}
                aria-label={label}
                className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all active:scale-95 ${viewMode === id ? 'bg-white dark:bg-brand-charcoal-dark shadow-sm text-brand-charcoal-dark dark:text-white' : 'text-gray-400 hover:text-brand-charcoal dark:hover:text-white'}`}>
                <Icon size={15} />
              </button>
            ))}
          </div>
        ...
        <div className="flex gap-2 pb-1 mb-3 overflow-x-auto scrollbar-none">
          {CATEGORIES.map(({ id, label, emoji }) => (
            <button key={id} type="button"
              ...
              className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl border-2 text-sm font-bold whitespace-nowrap transition-all shrink-0 active:scale-[0.97]
                ${filters.category === id
                ...
                {paginated.map(prop => (
                  <div key={prop.id} className="relative group">
                    ...
                    <button
                      ...
                      className="absolute z-10 flex items-center justify-center w-10 h-10 transition-all rounded-full shadow-md top-3 right-3 bg-white/90 dark:bg-brand-charcoal-dark/90 hover:scale-110 active:scale-95">
                      <Heart size={15}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-8">
                <button type="button" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                  className="flex items-center justify-center transition-colors border border-gray-200 w-10 h-10 rounded-xl dark:border-white/10 text-brand-charcoal dark:text-white disabled:opacity-30 hover:border-brand-gold active:scale-95">
                  <ChevronLeft size={16} />
                </button>
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button key={pageNum} type="button" onClick={() => setPage(pageNum)}
                      className={`w-10 h-10 rounded-xl text-sm font-bold transition-all active:scale-95 ${page === pageNum ? 'bg-brand-gold text-white' : 'border border-gray-200 dark:border-white/10 text-brand-charcoal dark:text-white hover:border-brand-gold'}`}>
                      {pageNum}
                    </button>
                  );
                })}
