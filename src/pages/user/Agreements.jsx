...
const agreementTypes = [
  { icon: '🏠', label: 'Rental Agreements', desc: 'Signed tenancy agreements and receipts', count: 0, color: 'bg-blue-50 dark:bg-blue-500/10' },
  { icon: '🏢', label: 'Purchase Agreements', desc: 'Proof of purchase and transfer docs', count: 0, color: 'bg-emerald-50 dark:bg-emerald-500/10' },
  { icon: '🛋️', label: 'Shortlet Contracts', desc: 'Shortlet booking confirmations', count: 0, color: 'bg-amber-50 dark:bg-amber-500/10' },
  { icon: '🔧', label: 'Service Contracts', desc: 'Agreements with service providers', count: 0, color: 'bg-purple-50 dark:bg-purple-500/10' },
];
  ...
  return (
    <div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {agreementTypes.map((type) => (
            <div key={type.label} className={`${type.color} rounded-2xl p-4`}>
              ...
              <p className="text-xs font-bold text-brand-charcoal-dark dark:text-white mt-2">{type.count} document{type.count !== 1 ? 's' : ''}</p>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card p-12 text-center">
          ...
          <h3 className="font-display font-semibold text-brand-charcoal-dark dark:text-white">No documents yet</h3>
