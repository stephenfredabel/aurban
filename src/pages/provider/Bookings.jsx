...
import {
  ...
  AlertCircle, MessageSquare, ChevronRight, Settings,
  User,
} from 'lucide-react';
...
/* ════════════════════════════════════════════════════════════
   PROVIDER BOOKINGS — Manage incoming inspections & appointments
   Route: /provider/bookings (via ProviderDashboard delegation)
════════════════════════════════════════════════════════════ */
...
const TABS = ['upcoming', 'pending', 'past', 'all'];

export default function ProviderBookings() {
  const { t }            = useTranslation();
  const { bookings, getUpcoming, getPast, getByStatus, updateStatus, cancelBooking } = useBooking();
  const { format: formatPrice }  = useCurrency();
  const [tab, setTab]    = useState('upcoming');

  useEffect(() => { document.title = t('booking.title', 'Bookings') + ' — Aurban Provider'; }, [t]);

  const filtered = useMemo(() => {
    if (tab === 'upcoming') return getUpcoming;
    if (tab === 'pending')  return getByStatus('pending');
    if (tab === 'past')     return getPast;
    return bookings;
  }, [tab, getUpcoming, getPast, getByStatus, bookings]);

  const handleAccept = (id) => updateStatus(id, 'confirmed');
  const handleDecline = (id) => cancelBooking(id);
  const handleRequestCompletion = (id) => updateStatus(id, 'completed');
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-400 mb-5">Manage your inspections and appointments</p>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-card p-12 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1 max-w-xs mx-auto">
            When clients book inspections, they'll appear here
          </p>
          <Link to="/provider/settings"
            ...
            <Settings size={14} /> {t('booking.setAvailability', 'Set Your Availability')}
          </Link>
        </div>
      ) : (
            ...
            return (
              <div key={booking.id}
                className={`bg-white dark:bg-gray-900 rounded-2xl shadow-card p-4 sm:p-5 transition-shadow hover:shadow-md ${
                  booking.status === 'pending' ? 'ring-2 ring-amber-200 dark:ring-amber-500/30' : ''
                }`}>
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5 pl-8">{booking.listingTitle}</p>
                  </div>
                ...
                {/* Client notes */}
                {booking.notes && (
                  <div className="px-3 py-2 mb-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      <span className="font-bold text-gray-600 dark:text-gray-300">Note:</span> {booking.notes}
                    </p>
                  </div>
                )}
                ...
                {booking.escrowAmount > 0 && (
                  <div className="flex items-center gap-2 px-3 py-2 mb-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20">
                    <AlertCircle size={13} className="text-emerald-500 shrink-0" />
                    <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-300">
                      {t('booking.escrowHeld', 'Held in Escrow')}: {formatPrice(booking.escrowAmount)}
                    </span>
                  </div>
                )}
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-white/10">
                  {booking.status === 'pending' && (
                    <>
                      <button onClick={() => handleAccept(booking.id)}
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-white bg-emerald-500 hover:bg-emerald-600 rounded-xl transition-colors">
                        <CheckCircle2 size={13} /> {t('booking.accept', 'Accept')}
                      </button>
                      <button onClick={() => handleDecline(booking.id)}
                        ...
                        <XCircle size={13} /> {t('booking.decline', 'Decline')}
                      </button>
                    </>
                  )}
                  ...
                  {booking.status === 'in_progress' && (
                    <button onClick={() => handleRequestCompletion(booking.id)}
                      ...
                      <CheckCircle2 size={13} /> {t('booking.requestCompletion', 'Request Completion')}
                    </button>
                  )}
                  <Link to="/provider/messages"
                    ...
                    <MessageSquare size={12} /> Message <ChevronRight size={12} />
                  </Link>
                </div>
              </div>
            );
